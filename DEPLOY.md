...
## Админка исследований (/admin)

Статьи для страницы `/research` пишутся через `/admin/research` (визуальный редактор, доступен с телефона и с компьютера). Доступ защищён одним паролем.

На сервере в файле `.env` (в той же папке, что и `docker-compose.yml`) должна быть переменная:

```env
ADMIN_PASSWORD=придумайте_длинный_пароль
```

Файл `.env` не синхронизируется деплоем (исключён из rsync), поэтому его нужно один раз создать/отредактировать вручную на сервере. Статьи хранятся в отдельном Docker-томе `arvexo-data`, который переживает передеплой.
...
`PORT` можно не задавать, тогда workflow использует `22`. Если задаете `PORT`, значение должно быть только цифрами, без пробелов и переносов строк.
